...
<div class="xs-crumb clearfix">
    ...
    <ul id="J_crumbBar" class="nav">
        ...
        <#assign clkstat="atype=breadclk|stype=list|sattr=${((selectedCatalog.catalogName)!'')?url}"/>
            ...
            <li><span><a clkstat="${clkstat}bread_cat=${catalog.catalogName?url}|bread_lv=${catalog_index+1}" href="<@xurl value="/ebay-list-action.htm?cat=${catalog.catalogId}&vm=${(params.vm)!''}&p_code=${(fatParams.providerCode)!''}&s_id=${(s_id)!''}" />" alt="${catalog.catalogName!}"><#if (catalog.catalogName??)><#if (catalog.catalogName?length>20)>${catalog.catalogName?substring(0,20)}...<#else>${catalog.catalogName}</#if></#if></a></span><i></i></li>
            </#list>
        </#if>
    </ul>
	...
	<div id="J_facet_box" class="facet-bd">
		<style type="text/css">
	 	...
	 	<#if (fatParams.catalogId)??>
        	<#assign urlTemp="/ebay-list-action.htm?cat=${(fatParams.catalogId)!''}&p_code=${(fatParams.providerCode)!''}&mkt=${(params.mkt)!''}&s_id=${(s_id)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
                    ...
                    <div class="normal-item-box clearfix">
                        <div class="facet-item-box">
							<ul class="clearfix">
	                            ...
	                            <#elseif selectFacetBo.facetType == 'PRICE' && (selectFacetBo.facetValueBos)?? && (selectFacetBo.facetValueBos?size gt 0) >
	                            ...
	                            <#else>
	                            	<#if selectFacetBo?? && selectFacetBo.facetValueBos[0]?? && (selectFacetBo.facetValueBos)?? && (selectFacetBo.facetValueBos?size gt 0)>
		...
		<#if selectedCatalogTree?? && selectedCatalogTree.childCatalog?? && selectedCatalogTree.childCatalog?size gt 0>
	        <#assign urlTemp="/ebay-list-action.htm?p_code=${(fatParams.providerCode)!''}&mkt=${(params.mkt)!''}&s_id=${(s_id)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
	        ...
	        <div class="facet-line brand-facet-box brand-facet-default-status">
	            ...
	            <div class="line-bd">
	                <div class="brand-item-box multiple-item-box">
	                	...
	                	<#assign urlTemp="/ebay-list-action.htm?cat=${(fatParams.catalogId)!''}&p_code=${(fatParams.providerCode)!''}&mkt=${(params.mkt)!''}&s_id=${(s_id)!''}&f_price=${(fatParams.priceRangeEnum.order)!''}&s_price=${(fatParams.startPrice)!''}&e_price=${(fatParams.endPrice)!''}&filter=${(fatParams.filter)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
	                    ...
	                    <ul id="J_brandBoxUl">
	                    	<#list facetFilterValueBoList as facetFilterValueBo >
		                    	<li>
			                    	<a title="${facetFilterValueBo.name?html}" dataCollect="type=brand|id=${(facetFilterValueBo.id)!}" data-brandid="${facetFilterValueBo.id}" onclick="LEON.multiSelectItemAction(event,this);" href="<@xurl value="${urlTemp}&bid=${facetFilterValueBo.id}" />">
			...
			<#--
			尺码:<a href="<@xurl value='/ebay-list-action.htm?cat=100000223&filter=-181;-476'/>">尺码测试</a>
			-->
	        <!-- 其他(尺码，材料。。。。等等) -->
	        <script>
	        ...
	        <#if (attrFacetBoList)??>
	        <#assign urlTemp="/ebay-list-action.htm?cat=${(fatParams.catalogId)!''}&p_code=${(fatParams.providerCode)!''}&mkt=${(params.mkt)!''}&s_id=${(s_id)!''}&f_price=${(fatParams.priceRangeEnum.order)!''}&s_price=${(fatParams.startPrice)!''}&e_price=${(fatParams.endPrice)!''}&bid=${(params.bid)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
	        <#list attrFacetBoList as attrFacet>
		        ...
		        <#if attrFacet.facetType == 'PRICE'>
			        <#assign urlTemp1="/ebay-list-action.htm?cat=${(fatParams.catalogId)!''}&bid=${(params.bid)!''}&filter=${(fatParams.filter)!''}&p_code=${(fatParams.providerCode)!''}&s_id=${(s_id)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
...
<div class="xs-mainsection clearfix">
	...
	<div class="xs-filterbox clearfix">
		<div class="xs-toolbar">
            <div class="showmode">
                <#assign urlTemp="/ebay-list-action.htm?cat=${(fatParams.catalogId)!''}&bid=${(params.bid)!''}&sort=${fatParams.sort.sortOrder}&f_price=${(fatParams.priceRangeEnum.order)!''}&s_price=${(fatParams.startPrice)!''}&e_price=${(fatParams.endPrice)!''}&filter=${(fatParams.filter)!''}&ba=${(params.ba)!''}&p_code=${(fatParams.providerCode)!''}&mkt=${(params.mkt)!''}&s_id=${(s_id)!''}&channel=${(params.channel)!''}">
            ...
            <div class="xs-sort-bar">
                <#assign urlTemp="/ebay-list-action.htm?cat=${(fatParams.catalogId)!''}&bid=${(params.bid)!''}&vm=${(params.vm)!''}&f_price=${(fatParams.priceRangeEnum.order)!''}&s_price=${(fatParams.startPrice)!''}&e_price=${(fatParams.endPrice)!''}&filter=${(fatParams.filter)!''}&ba=${(params.ba)!''}&p_code=${(fatParams.providerCode)!''}&mkt=${(params.mkt)!''}&s_id=${(s_id)!''}&channel=${(params.channel)!''}">
            ...
            <#--
            <div class="fh-box">
            	<#assign urlTemp="/ebay-list-action.htm?cat=${(fatParams.catalogId)!''}&bid=${(params.bid)!''}&vm=${(params.vm)!''}&f_price=${(fatParams.priceRangeEnum.order)!''}&s_price=${(fatParams.startPrice)!''}&e_price=${(fatParams.endPrice)!''}&filter=${(fatParams.filter)!''}&ba=${(params.ba)!''}&p_code=${(fatParams.providerCode)!''}&mkt=${(params.mkt)!''}&s_id=${(s_id)!''}&sort=${(params.sort)!''}">
            	<label>发货方式：</label>
            ...
            -->
            <@xpage currentPage="${(page.pageNo)!0}" totalPage="${(page.pageCount)!0}" repQuery="" theme="1" />
                <!--
                 <ul>
			...
			<div id="J_itemListBox" class="xs-itemsbox">
	    		...
	    		<ul class="xs-itemsul-detail clearfix">
		    		...
		    		<li class="item" data-itemid="${goods.urlId}" >
		            	<ol>
		                    ...
		                    <li class="tit"><a clkstat="${clkstat}" href="${((fatParams.mktType??)&&fatParams.mktType=='EBAY')?string(ebayUrl,itemUrl)}product/${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}" target="_blank">${goods.nameHighlight}</a></li>
		                    <li class="price">
		                    ...
		                    <li class="btnbar">
		                    	<#if goods.buyNow>
		                    		<a clkstat="atype=immbuy|stype=list|sattr=${((selectedCatalog.catalogName)!'')?url}|clk_loc=${30*(page.pageNo-1)+(goods_index+1)}|itemid=${goods.urlId}" target="_blank" href="${((fatParams.mktType??)&&fatParams.mktType=='EBAY')?string(ebayUrl,itemUrl)}product/${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}" class="onsale-btn">立即购买</a>
		                    	</#if>
	    		...
	    		<ul class="xs-itemsul-img clearfix">
	            	...
	            	<li class="item" data-itemid="${goods.urlId}" <#if goods_index % 4 ==3>style=" margin-right:0;"</#if> >
	                	<ol>
	                    	<li class="pic"><a clkstat="${clkstat}" href="${((fatParams.mktType??)&&fatParams.mktType=='EBAY')?string(ebayUrl,itemUrl)}product/${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}" target="_blank"><img original="${imagesUrl}${goods.imgEbayUrl220}" /><#if goods.soldOut><b class="saleout-ico"></b></#if></a></li>
	                        <li class="tit"><a clkstat="${clkstat}" href="${((fatParams.mktType??)&&fatParams.mktType=='EBAY')?string(ebayUrl,itemUrl)}product/${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}" target="_blank">${goods.nameHighlight}</a></li>
	                        <li class="price">
...
<#-- 推荐引擎/百分点 -->
LEON.xreData({brandId : "${(recommenBrandIds)!}",catId : "${recommendCatIds!}",itemId : "${recommendItemIds!}",stype : "list",sattr : "${((selectedCatalog.catalogName)!'')?url}",siteId : "1001",channelId : "1002",displayTop10:false});
<#-- 点击流事件绑定 -->
$('#J_itemListBox,#J_sortBox,#J_crumbBar,#J_catalogTire,#J_facet_box,#J_filterbox').clkBindMethod();
})();
</script>
<script src="${staticUrl}static/searchstatic/script/commentinfo.js" type="text/javascript"></script>
