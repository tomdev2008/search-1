...
<title>【${params.kw?html}】价格,款式,购买查询-走秀网</title>
<meta name="description" content="在走秀网中找到了${page.recordCount}件${params.kw?html}相关的商品；网上购买${params.kw?html}就上走秀网！" />
<meta name="keywords" content="${params.kw?html},走秀网${params.kw?html}" />
...
<div class="xs-crumb clearfix">
	<label class="hd">您的位置：</label>
	     ...
	     <ul id="J_crumbBar" class="tabs">
		    <li class="no_dropdown first"><a href='<@xurl value="/search-action.htm?kw=${searchKW?url}" />'>全部结果</a><i></i></li>
		    <#if selectCatalogPlaneList??>
			    <#assign clkstat="atype=breadclk|stype=kw|sattr=${params.kw?url}|"/>
			    <#assign catUrlTemp="/search-action.htm?kw=${searchKW?url}&mkt=${(fatParams.mktType.type)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
		    ...
		    <li class="no_dropdown"><span class="no_active_label">${searchKW?html}</span></li>
		</ul>
...
<#if termErrorCorrect??>
	<div class="xs-errortermbox">
	    <p class="errmsg">
	            抱歉！指定范围没有找到“<span class="hl">${params.kw?html}</span>”商品，在全站为您找到了与“<span class="hl">${termErrorCorrect?html}</span>”相关的商品。
	    </p>
	</div>
	<#elseif termAutoCorrect??>
	<div class="xs-errortermbox">
	    <p class="errmsg">
	            您要找的是不是：“<a href="<@xurl value="/search-action.htm?kw=${termAutoCorrect?url}&itemshowtype=${(params.itemshowtype)!''}" />" class="hl">${termAutoCorrect?html}</a>”
	    </p>
	</div>
	...
	<div class="xs-relatetermbox xs-bg">
	    ...
	    <#list relatedTerms as term>
	        <a class="${(term_index==0)?string('first','')}" href="${searchUrl}s?kw=${term?url}&itemshowtype=${(params.itemshowtype)!''}" clkstat="${clkstat}${term?url}">${term?html}</a>
	    </#list>
	    </div>
	</div>
</#if>
	...
	<div  id="J_facet_box" class="facet-bd">
		...
		<#if !((fatParams.catalogId)??)>
			<#if catalogBoList?? && catalogBoList?size gt 0>
				<#assign urlTemp="/search-action.htm?kw=${searchKW?url}&mkt=${(fatParams.mktType.type)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
		...
		<#else>
                    ...
                    <div class="line-bd">
                        <div class="normal-item-box clearfix">
                        	<div class="facet-item-box">
	                            <ul class="clearfix">
	                            	<#--去掉selectedCatalogTree的catalogName by lvshuding 201405051005>
		                            ...
		                            <#if selectFacetBoList?? && selectFacetBoList?size gt 0>
			                            ...
			                            <li><a href="<@xurl value="${urlTemp}&cat=${(fatParams.catalogId)!''}&filter=${(fatParams.filter)!''}&bid=${(params.bid)!''}" />">${selectFacetBo.facetDisplay}：${selectFacetBo.facetValueBos[0].name?html}<b class="close-ico"></b></a></li>
			                            <#else>
			                            <li><a href="<@xurl value="${urlTemp}&cat=${(fatParams.catalogId)!''}&bid=${(params.bid)!''}&f_price=${(fatParams.priceRangeEnum.order)!''}&filter=${(selectFacetBo.cancelFilter)!''}&s_price=${(fatParams.startPrice)!''}&e_price=${(fatParams.endPrice)!''}" />">${selectFacetBo.facetDisplay}：${selectFacetBo.facetValueBos[0].name?html}<b class="close-ico"></b></a></li>
			                            </#if>
			                            </#list>
		                            </#if>
		                            <li class="reset">
            ...
            <#if selectedCatalogTree?? && selectedCatalogTree.childCatalog?? && selectedCatalogTree.childCatalog?size gt 0 && selectCatalogPlaneList?? && selectCatalogPlaneList?size lt 3>
		        <#assign urlTemp="/search-action.htm?kw=${searchKW?url}&mkt=${(params.mkt)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
			                    ...
			                    <div>
			                        <ul id="J_catSecondItem" class="clearfix">
			                        	<#list selectedCatalogTree.childCatalog as catalog>
			                            	<li><a data-catalogid="${catalog.catalogId}" dataCollect="type=list|id=${(catalog.catalogId)!}" clkstat="${clkstat!},${catalog.catalogName?url}|cat_lv=2" href="<@xurl value='${urlTemp}&cat=${catalog.catalogId}' />" class="${(catalog.selected )?string('active','')}" >${catalog.catalogName?html}</a></li>
			                            </#list>
			                        </ul>
			                    </div>
			                </div>
	        ...
	        <div class="facet-line brand-facet-box brand-facet-default-status">
	            ...
	            <div class="line-bd">
	                <div id="J_itemboxbrand" class="brand-item-box multiple-item-box">
	                	...
	                	<#assign urlTemp="/search-action.htm?kw=${searchKW?url}&cat=${(fatParams.catalogId)!''}&mkt=${(fatParams.mktType.type)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}&s_price=${(fatParams.startPrice)!''}&e_price=${(fatParams.endPrice)!''}&f_price=${(fatParams.priceRangeEnum.order)!''}&filter=${(fatParams.filter)!''}">
	                    ...
	                    <ul id="J_brandBoxUl">
	                    	<#list facetFilterValueBoList as facetFilterValueBo >
		                    	<li>
			                    	<a clkstat="${clkstat}${facetFilterValueBo.name?url}" dataCollect="type=brand|id=${(facetFilterValueBo.id)!}" title="${facetFilterValueBo.name?html}" data-brandid="${facetFilterValueBo.id}" onclick="LEON.multiSelectItemAction(event,this);" href="<@xurl value="${urlTemp}&s_price=${(fatParams.startPrice)!''}&e_price=${(fatParams.endPrice)!''}&bid=${facetFilterValueBo.id}&f_price=${(fatParams.priceRangeEnum.order)!''}&filter=${(fatParams.filter)!''}" />">
	        ...
	        <#list attrFacetBoList as attrFacet>
		        ...
		        <#if attrFacet.facetType == 'PRICE'>
			        <#assign urlTemp1="/search-action.htm?kw=${searchKW?url}&cat=${(fatParams.catalogId)!''}&bid=${(params.bid)!''}&mkt=${(fatParams.mktType.type)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
	...
	<div class="xs-filterbox clearfix">
		<div class="xs-toolbar">
            <div class="showmode">
                ...
                -->
                <#assign urlTemp="/search-action.htm?kw=${searchKW?url}&cat=${(fatParams.catalogId)!''}&bid=${(params.bid)!''}&f_price=${(fatParams.priceRangeEnum.order)!''}&s_price=${(fatParams.startPrice)!''}&e_price=${(fatParams.endPrice)!''}&filter=${(fatParams.filter)!''}&ba=${(params.ba)!''}&sort=${fatParams.sort.sortOrder}&mkt=${(fatParams.mktType.type)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&channel=${(params.channel)!''}">
               	
               	<#--
                <#assign urlTemp="/search-action.htm?kw=${searchKW?url}&cat=${(fatParams.catalogId)!''}&bid=${(params.bid)!''}&f_price=${(fatParams.priceRangeEnum.order)!''}&s_price=${(fatParams.startPrice)!''}&e_price=${(fatParams.endPrice)!''}&filter=${(fatParams.filter)!''}&ba=${(params.ba)!''}&sort=${fatParams.sort.sortOrder}&mkt=${(fatParams.mktType.type)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}">
            ...
            <div class="xs-sort-bar">
                ...
                <dl id="J_sortBox" class="sort-box">
                    ...
                    value="${urlTemp}&sort=9
                    -->
             		<dd><a rel="nofollow" clkstat="${clkstat}9" href="<@xurl  value="${urlTemp}&sort=9" />" class="nonearr ${(fatParams.sort=='SCORE_AMOUNT_DESC')?string('active','')}">默认</a></dd>
                    ...
                    <dd><a rel="nofollow" clkstat="${clkstat}${(fatParams.sort=='ONSALE_TIME_DESC')?string('7','8')}" href="<@xurl  value="${urlTemp}&sort=${(fatParams.sort=='ONSALE_TIME_DESC')?string('7','8')}" />" class="${(fatParams.sort=='ONSALE_TIME_DESC'|| fatParams.sort=='ONSALE_TIME_ASC')?string('active','')}">上架时间<i class="${(fatParams.sort=='ONSALE_TIME_ASC')?string('','down')}"></i></a></dd>
                </dl>
            </div>
            ...
            <div class="fh-box">
                ...
                <ul>
                	<li><label for="J_All"><input id="J_All" name="Itemshowtype" type="radio" value="2|3" onClick="shouItemshowtype('')" checked = "checked">全部</label></li>
            ...
            <@xpage currentPage="${(page.pageNo)!0}" totalPage="${(page.pageCount)!0}" repQuery="kw=${searchKW?url}&mkt=${(fatParams.mktType.type)!''}" theme="1" />
            	<!--
                 <ul>
	    ...
	    <div id="J_filterbox" class="xs-main">
			...
			<div id="J_itemListBox" class="xs-itemsbox" style="padding-bottom:20px;">
	    		...
	    		<ul class="xs-itemsul-detail clearfix">
	    			...
	    			<#assign clkstat="atype=itemclk|stype=kw|sattr=${params.kw?url}|clk_loc=${30*(page.pageNo-1)+(goods_index+1)}|itemid=${goods.urlId}">
		    		<li class="item" data-itemid="${goods.urlId}" >
		            	<ol>
		                    ...
		                    <li class="btnbar">
		                    	<#if goods.buyNow>
		                    		<a clkstat="atype=immbuy|stype=kw|sattr=${params.kw?url}|clk_loc=${30*(page.pageNo-1)+(goods_index+1)}|itemid=${goods.urlId}" target="_blank" href="${getItemUrl(goods.providerCode!)}${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}" class="onsale-btn">立即购买</a>
		                    	</#if>
	    		...
	    		<ul class="xs-itemsul-img clearfix">
	            	...
	            	<li class="item" data-itemid="${goods.urlId}" <#if goods_index % 4 ==3>style=" margin-right:0;"</#if> >
	                	<ol>
	                        ...
	                        <!--<li class="btnbar">
	                        	<#if goods.buyNow>
	                        		<a rel="nofollow" clkstat="atype=immbuy|stype=kw|sattr=${params.kw?url}|clk_loc=${30*(page.pageNo-1)+(goods_index+1)}|itemid=${goods.urlId}" target="_blank" href="${getItemUrl(goods.providerCode!)}${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}" class="onsale-btn">立即购买</a>
	                        	</#if>
	                        	<#if goods.arrivalNotice>
	                        		<a clkstat="${clkstat}" target="_blank" href="${getItemUrl(goods.providerCode!)}${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}" class="offsale-btn">到货通知</a>
	                        	</#if>
	        ...
	        <div id="J_bottomPageBox" class="xs-pagebar clearfix">
	            <@xpage currentPage="${(page.pageNo)!0}" totalPage="${(page.pageCount)!0}" repQuery="kw=${searchKW?url}&mkt=${(fatParams.mktType.type)!''}" pageParamName="p"/>
	        </div>
	        <#-- 底部翻页结束 -->
	    </div>	    
	    <script>
...
LEON.xreDataCollect({siteId:1001,channelId:1001,kw:"${params.kw?url}",result:${page.recordCount!0}});
<#-- 点击流底部翻页@007 -->
