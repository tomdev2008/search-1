...
<script src="http://www.xiustatic.com/static/build/js/xiu/module/widget/brandLogoWidget.min.js?__=${xversion('/static/build/js/xiu/module/widget/brandLogoWidget.min.js')}" type="text/javascript"></script>
<script><#-- init info -->
$(document).ready(function(){$(".pic img").lazyload({placeholder:"http://www.xiu.com/static/img/default/default.234_312.jpg"});});

//品牌介绍
jQuery(function(){
    var eventDom = jQuery("#J_brandBox");
    xiu.module.widget.brandLogoWidget.Instance(eventDom);
});
...
<#if (bannerImageName2??)&&(""!=bannerImageName2)>
	<div id="brand_pic_div" class="brand_banner">
...
<div class="xs-crumb clearfix">
	<label class="hd">您的位置：</label>
        <ul id="J_crumbBar" class="tabs">
		   	<li class="no_dropdown first"><a href='<@xurl value="/brand-action.htm?bid=${(params.bid)!}" />'>全部结果</a><i></i></li>
		    <#if selectCatalogPlaneList??>
			    <#assign clkstat="atype=breadclk|stype=brand|sattr=${(brandName!)?url}">
			    <#assign catUrlTemp="/brand-action.htm?bid=${(params.bid)!}&mkt=${(fatParams.mktType.type)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
	...
	<#if ((!bannerImageName2??)||("" == bannerImageName2)) && ((story??)&&story?length gt 0)>
		<div id="J_brandBox" class="pp_info">
		    <div class="text_nr">
		        ...
		        <a href="${commentUrl}commentlist/brand/${(params.bid)!}.html" target="_blank"><img width=100 height=50 src="${imagesUrl}UploadFiles/xiu/brand${storeImgUrl!}" alt="${(brandName!)}" /></a>
		        <#else>
		        <a href="${commentUrl}commentlist/brand/${(params.bid)!}.html" target="_blank"><img src="${wwwUrl}static/ebay/images/brand-logo.png" alt="${(brandName!)}" /></a>
		        </#if>
	...
	<div id="J_facet_box" class="facet-bd">
		...
		<#if !((fatParams.catalogId)??)>
			<#if catalogBoList?? && catalogBoList?size gt 0>
				...
				<#assign urlTemp="/brand-action.htm?bid=${(params.bid)!}&mkt=${(fatParams.mktType.type)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
		...
		<#else>
        	<#assign urlTemp="/brand-action.htm?bid=${(params.bid)!}&mkt=${(fatParams.mktType.type)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">	        <#-- 您已选择 -->
	                ...
	                <div class="line-bd">
	                    <div class="normal-item-box clearfix">
	                    	<div class="facet-item-box">
	                            <ul class="clearfix">
	                            	<#--去掉selectedCatalogTree的catalogName by dingchenghong 20140428>
	        ...
	        <#assign urlTemp="/brand-action.htm?bid=${params.bid!}&mkt=${(params.mkt)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
	        ...
	        <#list attrFacetBoList as attrFacet>
		        ...
		        <#if attrFacet.facetType == 'PRICE'>
			        <#assign urlTemp1="/brand-action.htm?bid=${(params.bid)!}&cat=${(fatParams.catalogId)!''}&filter=${(fatParams.filter)!''}&mkt=${(fatParams.mktType.type)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&sort=${fatParams.sort.sortOrder}&vm=${params.vm!''}&channel=${(params.channel)!''}">
						    ...
						    <div class="normal-item-box ${(index % 2==1)?string('','multiple-item2-box')} clearfix">
						        <div id="J_attrBoxMargin_${attrFacet.facetId}" class="attrBoxMargin">
						            <ul id="J_attrBoxUl_${attrFacet.facetId}" class="clearfix" data-attrid="${attrFacet.facetId}">
					                    ...
					                    <span>${valueFacet.name?html}</span>
					                    <b></b>
					                    </a></li>
					                    </#list>
				                    </ul>
...
<div class="xs-mainsection clearfix">
	...
	<div class="xs-filterbox clearfix">
		<div class="xs-toolbar">
            <div class="showmode">
                <#assign urlTemp="/brand-action.htm?bid=${(params.bid)!''}&cat=${(fatParams.catalogId)!''}&sort=${fatParams.sort.sortOrder}&f_price=${(fatParams.priceRangeEnum.order)!''}&s_price=${(fatParams.startPrice)!''}&e_price=${(fatParams.endPrice)!''}&filter=${(fatParams.filter)!''}&ba=${(params.ba)!''}&mkt=${(fatParams.mktType.type)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&channel=${(params.channel)!''}">
            ...
            <div class="xs-sort-bar">
                <#assign urlTemp="/brand-action.htm?bid=${(params.bid)!}&cat=${(fatParams.catalogId)!''}&f_price=${(fatParams.priceRangeEnum.order)!''}&s_price=${(fatParams.startPrice)!''}&e_price=${(fatParams.endPrice)!''}&vm=${params.vm!''}&mkt=${(fatParams.mktType.type)!''}&s_id=${(s_id)!''}&itemshowtype=${(params.itemshowtype)!''}&channel=${(params.channel)!''}">
                ...
                <dl id="J_sortBox" class="sort-box">
            		...
            		<#if fatParams.catalogId?? && fatParams.catalogId gt 0>
                    ...
                    <dd><a rel="nofollow" clkstat="${clkstat}${(fatParams.sort=='PRICE_ASC')?string('2','1')}" href="<@xurl  value="${urlTemp}&ba=${(params.ba)!0}&sort=${(fatParams.sort=='PRICE_ASC')?string('2','1')}" />" class="${(fatParams.sort=='PRICE_ASC' || fatParams.sort=='PRICE_DESC')?string('active','')}">价格<i class="${(fatParams.sort=='PRICE_DESC')?string('down','')}"></i></a></dd>
                    <dd><a rel="nofollow" clkstat="${clkstat}${(fatParams.sort=='DISCOUNT_DESC')?string('3','4')}" href="<@xurl  value="${urlTemp}&ba=${(params.ba)!0}&sort=${(fatParams.sort=='DISCOUNT_DESC')?string('3','4')}" />" class="${(fatParams.sort=='DISCOUNT_DESC' || fatParams.sort=='DISCOUNT_ASC')?string('active','')}">折扣<i class="${(fatParams.sort=='DISCOUNT_ASC')?string('','down')}"></i></a></dd>
                    <dd><a rel="nofollow" clkstat="${clkstat}${(fatParams.sort=='ONSALE_TIME_DESC')?string('7','8')}" href="<@xurl  value="${urlTemp}&ba=${(params.ba)!0}&sort=${(fatParams.sort=='ONSALE_TIME_DESC')?string('7','8')}" />" class="${(fatParams.sort=='ONSALE_TIME_DESC'|| fatParams.sort=='ONSALE_TIME_ASC')?string('active','')}">上架时间<i class="${(fatParams.sort=='ONSALE_TIME_ASC')?string('','down')}"></i></a></dd>
        	...
        	<#if (page)??>
            <@xpage currentPage="${(page.pageNo)!0}" totalPage="${(page.pageCount)!0}" repQuery="bid=${params.bid!}&mkt=${(fatParams.mktType.type)!''}&bo=${brandOnly!''}" theme="1" />
            </#if> 
                <!--
                 <ul>
    ...
    <div class="xs-section">
	    <div id="J_filterbox" class="xs-main">       
	    		...
	    		<ul class="xs-itemsul-detail clearfix">
		    		...
		    		<li class="item" data-itemid="${goods.urlId}">
		            	<ol>
		                    ...
		                    <li class="tit"><a  clkstat="${clkstat}" href="${getItemUrl(goods.providerCode!)}${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}" target="_blank">${goods.name?html!}</a></li>
		                    <li class="price">
		                    ...
		                    <li class="btnbar">
		                    	<#if goods.buyNow>
		                    		<a clkstat="atype=immbuy|stype=brand|sattr=${((brandname!)?url)}|clk_loc=${30*(page.pageNo-1)+(goods_index+1)}|itemid=${goods.urlId}" target="_blank" href="${getItemUrl(goods.providerCode!)}${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}" class="onsale-btn">立即购买</a>
		                    	</#if>
	    		...
	    		<ul class="xs-itemsul-img clearfix">
	            	...
	            	<li class="item" data-itemid="${goods.urlId}" <#if goods_index % 4 ==3>style=" margin-right:0;"</#if> >
	                	<ol>
	                    	...
	                    	<li class="pic">
	                    	<a clkstat="${clkstat}" goods_sn="${goods.xiuSn}" target="_blank" href="${getItemUrl(goods.providerCode!)}${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}"><img alt="${goods.name}" original="${zsurl()}${goods.imgUrl220}" /><#if goods.soldOut><b class="saleout-ico"></b></#if>
	                    	</a>
	                        ...
	                        <li class="tit">
	                          ...
	                          <a clkstat="${clkstat}" target="_blank" href="${getItemUrl(goods.providerCode!)}${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}">${goods.name?html!}</a>
	                        </li>
	                        <li class="price">
	                        ...
	                        <!--<li class="btnbar">
	                        	<#if goods.buyNow>
	                        		<a rel="nofollow" clkstat="atype=immbuy|stype=brand|sattr=${(brandName!)?url}|clk_loc=${30*(page.pageNo-1)+(goods_index+1)}|itemid=${goods.urlId}" target="_blank" href="${getItemUrl(goods.providerCode!)}${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}" class="onsale-btn">立即购买</a>
	                        	</#if>
	                        	<#if goods.arrivalNotice>
	                        		<a clkstat="${clkstat}" target="_blank" href="${getItemUrl(goods.providerCode!)}${goods.urlId}.shtml${((s_id??)&&s_id!='')?string('?s_id='+s_id!'','')}" class="offsale-btn">到货通知</a>
	                        	</#if>
	        ...
	        <div id="J_bottomPageBox" class="xs-pagebar clearfix">
	            <@xpage currentPage="${(page.pageNo)!0}" totalPage="${(page.pageCount)!0}" repQuery="bid=${params.bid!}&mkt=${(fatParams.mktType.type)!''}&bo=${brandOnly!''}" pageParamName="p"/>
	        </div>
